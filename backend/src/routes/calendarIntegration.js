...
router.post("/google/callback", authMiddleware, async (req, res) => {
  try {
    console.log("🔍 POST /google/callback - Début");
    ...
    // Obtenir les informations du profil utilisateur (avec fallback)
    let profile = {
    ...
    try {
      console.log("🔄 Tentative d'obtention du profil utilisateur...");
      const oauth2 = google.oauth2({ version: "v2", auth: oauth2Client });
      const { data: userProfile } = await oauth2.userinfo.get();
      profile = userProfile;
      console.log("✅ Profil utilisateur obtenu:", profile.email);
    } catch (profileError) {
      console.warn(
        "⚠️ Impossible d'obtenir le profil utilisateur, utilisation des valeurs par défaut:",
...
router.put("/integrations/:type/settings", authMiddleware, async (req, res) => {
  try {
...
router.post("/outlook/export-formation", authMiddleware, async (req, res) => {
  try {
    const { formationId } = req.body;
    const userId = req.user.id;
